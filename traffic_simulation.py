...
CAR_SPEED = 5 # Increased car speed for faster movement
MAX_CARS_PER_ZONE = 40 # Increased maximum cars per zone
...
class TrafficLight:
    def __init__(self, x, y):
        ...
        self.cycle_time = 3 # Slightly faster traffic light cycle (e.g., 3 seconds per state)
    ...
    <body class="p-4">
        ...
        <script>
            ...
            function initializeChart(labels, data) {
                // IMPORTANT: Destroy existing chart if it exists before creating a new one
                if (totalCarsChart) {
                    totalCarsChart.destroy();
                }
            ...
            function updateChart(newLabel, newData) {
                if (!totalCarsChart) {
                    // This should ideally not be hit after initial_data, but as a fallback
            ...
            eventSource.onmessage = function(event) {
                ...
                console.log("Received SSE update:", data); // IMPORTANT: Keep this log for debugging
                ...
                if (data.type === 'initial_data') {
                    ...
                    initializeChart(labels, carCounts); // Call initializeChart with initial data

                    // Initialize live simulation visualization with current state
                    // console.log("Initial simulation data:", data.full_sim_data); // Keep this log for debugging
                    ...
                    // console.log("Live update for zones:", data.zone_data); // Keep this log for debugging
            ...
            eventSource.onerror = function(err) {
                console.error("EventSource failed:", err); // IMPORTANT: Keep this log for debugging
                // This error is likely due to the Chart.js reinitialization loop.
                // We'll automatically try to re-establish the connection in production,
                // but for debugging, seeing the error is important.
                eventSource.close(); 
                // You could add a retry mechanism here, but first, let's fix the Chart.js issue.
            };
        ...
        while True:
            ...
            if current_simulation_step > last_step_sent or (current_simulation_step == 0 and last_step_sent == -1):
                ...
                payload = {
                    ...
                    'history': list(graph_data_history),
...
current_zone_bounds = (0, 0, ZONE_SIZE, ZONE_SIZE)
...
def worker_simulate_step():
    ...
    if random.random() < 0.4 and len(worker_vehicles) < MAX_CARS_PER_ZONE:
        ...
        if direction == 'E': 
            x, y = -CAR_SIZE, ZONE_SIZE/2 - CAR_SIZE/2
        elif direction == 'W': 
            x, y = ZONE_SIZE, ZONE_SIZE/2 - CAR_SIZE/2
        elif direction == 'S': 
            x, y = ZONE_SIZE/2 - CAR_SIZE/2, -CAR_SIZE
        elif direction == 'N': 
            x, y = ZONE_SIZE/2 - CAR_SIZE/2, ZONE_SIZE
        else:
    ...
    if random.random() < 0.05 and len(worker_vehicles) > 10:
